...
int 		menu(void) // Menu principal
{
	...
	printf("| 3.	Afficher les réservations pour un jeu			|\n");
	printf("| 4.	Saisir un nouveau jeu 					|\n");
	printf("| 5.	Saisir un nouvel emprunt				|\n");
	printf("| 6.	Saisir une nouvelle réservation				|\n"); 
	printf("| 7.	Retour d'un jeu						|\n");
	printf("| 8.	Annuler une réservation					|\n");
...
nodeType	*createNode(jeu	game) // crée un maillon de liste a partir du jeu passé en paramètre, next = NULL pour éviter les erreurs de segmentation
{
	...
	if (node == NULL)
	{
		printf("Problème d'allocation pour la liste chainee\n");
		exit(-1);
	}
...
int 		ChargementAdherents(adherent *tabAdh[], int max)  //charge le contenu du fichier adherent en mémoire 
{
	...
	while (!feof(flot))
	{
		...
		if (tabAdh[i] == NULL)
		{
			printf("Probleme d'allocation en memoire à la %de place pour les adherents",i);
			exit(-1);
		}
...
reservation *ChargementReservation(int *nbResa, int *idResa) // charge le contenu du fichier reservation en mémoire
{
	reservation *tabResa;
	int i;
	FILE *flot;
...
void    	triAlphaJeu(nodeType *head, emprunt tabEmp[], int nbEmp) // affiche les jeux disponibles dans l'ordre alphabétique (FONCTION N°1.1 du menu)
{
    ...
    if (headbis == NULL)
	{
		printf("\nProblème de copie de la liste de jeux\n");
		exit(-4);
	}
    current = headbis;
...
void 		printEmprunt(emprunt tabEmp[], char date[], int max) // affiche tout les emprunt en cours (FONCTION N° 2)
{
	int i, exp;
...
void 		printResa(reservation *tabResa, nodeType *head, int nbResa, int nbJeux) // affiche toute les reservation en cours en focntion du jeux donné (FONCTION N° 3)
{
	...
	system(CLEAN_SCREEN);
	printf("Quelle(s) resérvation(s) voulez-vous afficher ? ");
	scanf("%s%*c", nomJeu);
	upperLower(nomJeu);
...
int			rechercheAdherent(adherent *tabAdh[], char nom[], int nbAdh) // permet de savoir si l'adhérent existe ou non
{
	int i;
	for (i = 0; i < nbAdh; i++)
	{
...
void 		nouvelAdherent(adherent *tabAdh[], char date[], int *nbAdh, int max) // crée un nouvel adherent
{
	...
	printf("\nNouvel adherent ajouté avec succes\n");
...
empResa		*ajoutEmpResa(empResa tabEmpResa[], adherent *tabAdh[], int *nb,  int *id, int *nbMax, char date[], int i, int idJeu) // ajoute un/e emprunt/reservation en fonction de ce qui lui est passé en paramètre
{
	empResa *newtab;
...
emprunt 	*saisieNouvelEmprunt(adherent *tabAdh[], emprunt tabEmp[], nodeType* head, char date[], int *nbAdh, int *nbEmp, int *idEmp, int *nbEmpmax, int nbJeux, int max) // saisie les informations pour un nouvel emprunt (FONCTION N° 2)
{
	...
	if (idAdh == -1)
	{
		printf("cette personne n'a jamais été enregistrée, voulez vous l'ajouter ? (oui/non) : ");
		scanf("%s%*c", choix);
	...
	{	
		...
		{
			printf("Cet adherent à déjà emprunté ce jeu, voulez vous en choisir un autre ? (oui/non) : ");
			scanf("%s", choix);
			if (strcmp(choix, "oui") == 0)	
				goto Jeu;
			else
...
reservation *saisieNouvelResa(adherent *tabAdh[], reservation tabResa[], emprunt tabEmp[], nodeType* head, char date[], int *nbAdh, int *nbResa,  int *idResa, int *nbResaMax, int nbEmp, int nbJeux, int max) // saisie les informations pour une nouvelle reservation (FONCTION N° 3)
{
	...
	else
	{
		if (dateIns(tabAdh, date, idAdh) == -1)
		{
			printf("L'adherent %s %s est arrivé à expiration, a-t-il payé sa cotisation ? (oui/non) : \n\n",  tabAdh[idAdh]->nom, tabAdh[idAdh]->prenom);
			scanf("%s", choix);
			if (strcmp(choix, "oui") == 0)
			{	
				strcpy(tabAdh[idAdh]->dateIns, date);
	...
	Jeu :
	printf("Entrez le nom du jeu à reserver : ");
	scanf ("%s%*c", jeu);
	upperLower (jeu);
	...
	{	
		...
		{
			printf("Cet adherent a déjà réservé ce jeu, voulez vous en choisir un autre ? (oui/non) : ");
			scanf("%s", choix);
			if (strcmp(choix, "oui") == 0)	
				goto Jeu;
			else
...
emprunt		*RetourJeu(adherent *tabAdh[], emprunt tabEmp[], emprunt tabResa[], int *nbEmp, int *idEmp, int *nbResa, int nbAdh, char date[]) // retourne un jeu et regarde si une reservation peu passer en emprunt (FONCTION N°6)
{
	...
	if (idAdh == -1)
	{
		printf("\ncette personne n'a jamais été enregistrée, retour au menu ...\n");
		return tabEmp;
	}
	else 
	{
...
reservation	*AnnulationResa(adherent *tabAdh[], reservation tabResa[], int *nbResa, int nbAdh) // Annule une reservation (FONCTION N°7)
{
	...
	system(CLEAN_SCREEN);
	printf("Entrez le nom de l'emprunteur qui souhaite annuler une réservation : ");
	scanf("%s", nom);
	upper(nom);
	...
	else
	{
		...
		{		
			printf("Choissisez un numero de réservation pour cet adherent : ");
			scanf("%d", &val);

			if (tabResa[val-1].idAdherent != idAdh+1)	
				while (tabResa[val-1].idAdherent != idAdh+1)
				{
					printf("Votre numéro de réservation n'est pas correct pour cet adherent veuillez resaisir : ");
					scanf("%d", &val);
				}
...
void 		saveJeuxTxt(nodeType *head, int nbJeux) //sauvegarde la liste de jeux dans son fichier texte
{
	...
	fclose(flot);
	printf("Sauvegarde des jeux en texte terminée\n");
}
...
void 		saveEmpTxt(emprunt *tabEmp, int nbEmp) //sauvegarde le tableau d'emprunt dans son fichier texte
{
	...
	if (flot == NULL)
	{
		printf("Problème d'ouverture du fichier emprunt.txt en écriture");
		return;
	}
