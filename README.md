...
Eller permet de récupérer la date actuelle grâce au fonction **time()** et **strftime()** qui sont importer depuis la bibliothèque *time.h*.
...
Cette fonction lit tous les emprunts d'un fichier binaire. Elle prend en paramètre le tableau d'emprunts ou de réservations, un nombre d'emprunt ou réservation et un fichier binaire.
...
Cette fonction prend en paramètre la liste chaînée de jeu, le nombre de jeux / 2, une liste chaîné G ou D et une liste sauvegarde S.
...
Ensuite tant que les 2 listes ne sont pas *NULL* elle effectue un test pour copier chaque maillon 1 par 1 dans l'ordre dans notre liste fusionné.
Puis elle relance la fonction en envoyant la valeur *next* d'une des 2 liste pour l'attribbuer au maillon suivant de notre liste fusioné.
Une fois qu'une des 2 listes est à nul elle copie toujours defaçon récursif le reste de la liste G ou D. Elle retourne la liste fusionée à la fin.
...
Elle permet de trier notre liste par le principe de la fusion en faisant appelle à la fonction **copierL** et **fusionL**.
...
Cette fonction prend en paramètre la liste chaînée de jeu, le tableau d'emprunt, et le nombre total d'emprunt.
...
Cette fonction prend en paramètre le tableau d'adhérent et le nombre d'adhérents.
...
Cette fonction prend en paramètre le tableau d'adhérent, un nom, un prénom et le nombres d'adhérent.

Elle permet de chercher si le nom et prénom choisi appartienne à un adhérent ou non. Retourne sa position si trouvé ou -1 si il n'existe pas.
...
Cette fonction prend en paramètre le tableau d'adherent, la date du jours, la position de l'adherent et un pointeur int.
...
Cette fonction prend en paramètre le tableau d'adherent, l'id d'un adherent et le nombre d'emprunt.
...
Cette fonction  prend en paramètre le tableau d'emprunt, la date du jours, le nombre d'emprunts et l'id de l'adhérent.

Elle permet de savoir si un emprunt effectué par cet adhérent et en retard ou non en regardant si pour un emprunt contenant l'id de notre adhérent, la focntion **dateEmp** nous retourne -1.
...
Cette fonction prend en paramètre le tableau d'emprunt/réservation, l'id d'un jeu, l'id d'un adherent et le nombre d'emprunt.
...
Cette fonction prend en paramètre le tableau d'emprunt ou de réservation, le nombre d'emprunt ou de réservation, et un id.

Elle permet de savoir les id des emprunts ou réservation effecuté par un adhérent choisi.

<br/>	

	empResa 	*decalerGauche(empResa tabEmpResa[], int *nbEmpResa, int val);
Cette fonction prend en paramètre le tableau d'emprunt ou de réservation, la taille logique du tableau et un indice auquel commencer.
...
Cette fonction prend en paramètre le tableau de réservation, le nombre de réservation et l'id d'un jeu.

Elle permet de trouvé la première réservation sur un jeu donné. Elle *return* sa position ou -1 si elle n'en trouve pas.
...
Cette fonction prend en paramètre tout les tableaux et tout les nombres (jeux, adherent, emprunt, resa)

Elle demande si l'utilisateur veux changer de répertoire de sauvegarde pour les fichier txt ou non. Ce nouveau repertoir n'affectera que les fichiers txt et non les fichiers binaire. Puis fait appelle à la fonction **sauvergarder()** et toute les fonction de sauvegarde en Txt (**saveJeuxTxt()**, **saveAdhTxt()**, **saveEmpTxt()**, **saveResaTxt()**)
...
### Prototype fonction global
	void 		global(void);
Cette fonction est la fonction global, elle sert à :
